...
const FormasPagamento = ({ formas, pagamentosSelecionados, valores, diferencaPagamento, onAlternarForma, onAlterarValor, disabled = false }) => (
  <section className="bg-white border border-border rounded-lg p-5 space-y-5">
    <div>
      <h2 className="text-section font-semibold text-primary">Formas de pagamento</h2>
      <p className="text-sm text-gray-500">Selecione uma ou mais formas e informe o valor de cada uma.</p>
    </div>
    ...
    {diferencaPagamento > 0 && <p className="font-medium text-danger-text">Faltam {formatarMoeda(diferencaPagamento)}</p>}
    {diferencaPagamento < 0 && <p className="font-medium text-danger-text">Pagamento excede em {formatarMoeda(Math.abs(diferencaPagamento))}</p>}
    {diferencaPagamento === 0 && <p className="font-medium text-success">Pagamento completo.</p>}
  </section>
);

...
const ResumoCompra = ({ aluno, itens = [], armario, valorContribuicao = 0, total = 0 }) => {
  ...
  return (
    <aside className="w-full bg-surface border border-border rounded-lg p-5 xl:sticky xl:top-0 xl:self-start" aria-label="Resumo da compra">
      <h2 className="text-section font-semibold text-primary border-b border-border pb-3">Resumo da compra</h2>
      <div className="py-4 border-b border-border text-sm text-text-primary space-y-1">
        <p><span className="text-text-secondary">Nome:</span> <strong>{aluno.nome || "Aluno não identificado"}</strong></p>
        <p><span className="text-text-secondary">CPF:</span> <strong>{aluno.cpf || "—"}</strong></p>
      </div>
      <div className="py-4 border-b border-border text-sm space-y-2">
        ...
        {itens.map((item) => {
          ...
          return <p key={item.id} className="pl-2 text-xs text-text-secondary">
...
const ResumoLinha = ({ titulo, valor }) => <div className="flex justify-between gap-4 text-text-primary"><span>{titulo}</span><span>{valor}</span></div>;
